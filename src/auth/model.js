...
// 第二步：创建一个模型
var Auth = mongoose.model('User', authSchema);
 ...
 * 
 */
authSchema.methods.findUser = function(cb){
 ...
 * 
 */
authSchema.statics.findByName = function (name, cb) {// 注：静态方法，不能使用箭头函数
    return this.find({
...
authSchema.query.byName = function(name) {
...
authSchema.virtual('name').set(function (name) {
